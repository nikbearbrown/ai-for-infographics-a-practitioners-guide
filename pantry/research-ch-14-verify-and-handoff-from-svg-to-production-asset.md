# Research: Chapter 14 — Verify and Handoff: From SVG to Production Asset
## Brutalist SVG x Claude: Infographic Design and AI-Assisted Production

**Chapter one-line:** Students learn the Verify phase and the Handoff phase, including SVG to Illustrator, Illustrator to .ai, .ai to PNG, and the manual corrections that every AI-generated SVG requires.
...
### What is settled
Generated SVG must be verified as structure, text, style, naming, and handoff artifact before it is production-ready. Relevant sources for this settled ground include W3C SVG Working Group, Eisenberg and Bellamy-Royds, Adobe.

### What is disputed
Manual Illustrator correction can be essential, but it may reduce reproducibility if not documented. The chapter should distinguish evidence-based design rules from local editorial judgment, especially when examples depend on audience, medium, or toolchain.

### What has changed recently (last 5 years)
Automated SVG-to-PNG and handoff pipelines are easier to script, but human visual QA remains necessary. High-risk operational details include Claude Code behavior, D3 examples, WCAG guidance, Illustrator import behavior, and Node/SVG conversion scripts; these should be verified immediately before publication or course use.
...
Teach this chapter through failure-first comparison. The target reader can already ask an AI tool to produce SVG, so the instructional sequence should begin with a plausible AI-generated or professionally polished failure, then name the principle that explains the failure, then rebuild the artifact under constraint. Common misconceptions: polish equals correctness, prompts replace design specifications, and AI output can be judged by appearance alone. Students understand verification and production handoff when they can defend a design choice in language that survives a skeptical review, not when they can merely reproduce a pretty result.
